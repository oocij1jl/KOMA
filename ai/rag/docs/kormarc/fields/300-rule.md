...
biblio.page에서 수량을 뽑아 a에 기술한다. API 문자열에 장·책·권·면·매 단위가 있으면 그 단위를 따르고 없으면 p.를 쓴다. biblio.book_size가 mm이면 큰 값을 세로로 보고 cm로 올림해 c에 기술하고, cm이면 그대로 쓴다. 단위를 알 수 없으면 크기를 만들지 않는다. 수량과 크기가 모두 없으면 300을 만들지 않고 '형태사항 근거 없음'을 남긴다.
